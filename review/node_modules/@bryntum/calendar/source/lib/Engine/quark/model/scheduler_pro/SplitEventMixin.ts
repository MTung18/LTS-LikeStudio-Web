import { HasProposedValue, ProposedArgumentsOf, ProposedOrPrevious, ProposedOrPreviousValueOf, ProposedValueOf, WriteInfo, WriteSeveral, PreviousValueOf } from "../../../../ChronoGraph/chrono/Effect.js"
...
import { durationFormula, DurationVar, endDateFormula, EndDateVar, SEDDispatcher, startDateFormula, StartDateVar } from "../scheduler_basic/BaseEventDispatcher.js"
...
export const segmentsConverter = (value : Partial<SchedulerProEventSegment>[], record : SplitEventMixin) => record.processSegmentsValue(value)
...
export const SEDSGCycleDescription = CycleDescription.new({
...
export const SEDSGForwardCycleResolution = CycleResolution.new({
    description                 : SEDSGCycleDescription,
...
export class SplitEventMixin extends Mixin(
    ...
    class SplitEventMixin extends base {
        ...
        @model_field({
            type    : 'array',
            convert : segmentsConverter as (value : any, record : any) => any
        })
        ...
        * doWriteSegments (segments, writes? : WriteInfo[]) {
            writes = writes || []
                    ...
                    // if a segment overlaps the previous one
                        ...
                        if (previousSegment.graph) {
                            ...
                            const previousSegmentEndOffset   = keepDuration ?
                                ...
                                // otherwise just combine interseted segments by building a new [min start, max end] segment
        ...
        * calculateAdjustedSegments () {
            ...
            if (segments) {
                ...
                if (segments) {
                    segmentsSnapshot = this.getSegmentsSnapshot()
                }

                if (segmentsSnapshot !== this._lastSegmentsSnapshot) {
                    hasChanges                  = true
        ...
        processSegmentsValue (value : Partial<InstanceType<this['segmentModelClass']>>[]) : InstanceType<this['segmentModelClass']>[] | null {
            // by default reurn the value as is
        ...
        * calculateStartDateBySegments () : CalculationIterator<Date> {
            ...
            if (segments) {
                ...
                result                      = yield* this.skipNonWorkingTime(rawDate, true)
            }
        ...
        * calculateEndDateBySegments () : CalculationIterator<Date> {
            ...
            if (segments) {
                ...
                result                      = yield* this.skipNonWorkingTime(rawDate, false)
            }
        ...
        * useEventAvailabilityIterator () : CalculationIterator<boolean> {
            // always use availability iterator
            return true
        }
        ...
        /**
         * Indicates whether the event is segmented or not.
         * Returns true if the event is segmented and false otherwise.
         * @return `true` if the event is segmented and `false` otherwise.
         */
         get isSegmented () : boolean {
            const segments : InstanceType<this['segmentModelClass']>[] = this.segments
        ...
        async mergeSegments (segment1 : InstanceType<this['segmentModelClass']>, segment2 : InstanceType<this['segmentModelClass']>) {
            ...
            // merging itself will be done automatically inside of `calculateSegments`
        ...
        // Override storeFieldChange to support revertChanges for segmnts field
