...
const reportCriteria = computed(() => {
    ...
    const employeeFound = data.find(item => item.name === 'employees')
    if (employeeFound.value && employeeFound.value.length == 0) {
        employeeFound.value = JSON.parse(localStorage.getItem("employees"))
    }
    return data
})
